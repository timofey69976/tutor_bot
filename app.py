...
ФИНАЛЬНОЕ ИСПРАВЛЕНИЕ v5: Полная переработка системы кеширования и FSM +
ИСКЛЮЧЕНИЕ ЗАНЯТЫХ СЛОТОВ ИЗ РАСПИСАНИЯ (нельзя дважды занять одно и то же время)
"""
...
async def confirm_request_handler(callback: types.CallbackQuery, bot: Bot):
    ...
    # На всякий случай можно проверить ещё раз (опционально)
