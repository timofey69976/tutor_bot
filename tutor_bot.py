...
def subjects_keyboard(multiple=False):

	"""Клавиатура выбора предметов (ОДИНОЧНЫЙ ВЫБОР)"""
...
def validate_grade(grade_str: str) -> bool:
	...
	"""Проверить, что номер класса от 1 до 11"""
...
async def first_lesson_class_handler(message: types.Message, state: FSMContext):
	...
	"""Получить класс ученика - с валидацией от 1 до 11"""

	if await menu_button_handler(message, state):

		return

	if not validate_grade(message.text):

		await message.answer(

			"❌ Ошибка! Номер класса должен быть от 1 до 11.\n\nПожалуйста, введи корректный номер класса:",
	...
	await message.answer(
		...
		"Выбери предмет, которым ты будешь заниматься:",
...
async def repeat_lesson_handler(callback: types.CallbackQuery, state: FSMContext):
	...
	await callback.message.edit_text(
		...
		"Выбери предмет, которым ты занимаешься:",
...
async def subject_select_handler(callback: types.CallbackQuery, state: FSMContext):
	...
	"""Выбор предмета (ОДИНОЧНЫЙ ВЫБОР)"""

	subject = callback.data.replace("subject_", "")

	await state.update_data(selected_subjects=[subject])

	await callback.answer(f"✅ Выбран предмет: {subject}")
...
# ============ KEEP-ALIVE МЕХАНИЗМ (для Render Free план) ============

async def keep_alive(bot: Bot):

	"""Отправляет периодический сигнал каждые 10 минут для предотвращения засыпания приложения на Render"""
	...
	while True:
		...
		try:
			...
			# Проверяем время работы - отправляем log в консоль
			current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

			print(f"⏰ Keep-alive сигнал отправлен - {current_time}")

			# Дополнительно: опционально отправляем себе в Telegram уведомление
			# (закомментируйте, если не хотите получать сообщения)

			# await bot.send_message(

			# 	TUTOR_ID,

			# 	f"🤖 Бот активен - {current_time}"

			# )

		except Exception as e:

			print(f"❌ Ошибка в keep-alive механизме: {e}")
...
async def main():
	...
	keep_alive_task = asyncio.create_task(keep_alive(bot))

	print("🤖 Бот запущен! Ожидаем сообщений...")

	print("⏰ Keep-alive механизм активирован (сигнал каждые 10 минут)")
